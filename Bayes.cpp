	...
	if (BOOSTING){
		...
		while (!termine && pasadas < 6){
			...
			for (std::vector<std::vector<std::string> >::iterator it = rows.begin(); it !=rows.end(); ++it){
				...
				boost::regex regex("\\w+");
				boost::sregex_token_iterator iter(text.begin(), text.end(), regex, 0);
				long double first_try = evaluar(iter);
			...
			fprintf(stderr,"pass nr: %d, number of errors: %d \n",pasadas,errores); //pasadas = pass, errores: errors
			pasadas++;
		}
	}
}
...
std::vector<long double>* Bayes::Predicciones(){
	...
	char path[100];
	if (unlabeled){
		strcpy_s(path, "Data/unlabeledTrainData.tsv");
	} else {
		strcpy_s(path, "testData.tsv");
	}
	printf("Using file %s\n", path);
